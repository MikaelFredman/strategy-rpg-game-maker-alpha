...
1. Open the public [`0.2.0-beta.10` package](downloads/Strategy-RPG-Game-Maker-0.2.0-beta.10.zip).
2. Download `Strategy-RPG-Game-Maker-0.2.0-beta.10.zip` using GitHub's download button.
...
12. In a tactical battle, verify sequential half-second tile-by-tile movement: one character completes its route before the next begins. Confirm visible weapon-aware sword, spear, bow or staff actions originating at the acting figure. In AI spectator mode, arrows, fireballs and damage labels should complete before the next round begins.
13. In the panels below the tactical map, confirm that each side's opening maximum HP remains fixed after a casualty and that the fallen character stays in the list at 0 HP with red row text.
14. In an AI-only world simulation, inspect Latest world event for decisions to strike before reinforcements arrive, consolidate against a reinforced enemy or form a multi-group army.
15. Save a project with **Save Project As…**, reopen it with **Open**, and confirm that maps, links, story, rotations and animation poses remain intact.
16. Optionally start a second game with all controllers set to AI and watch the spectator simulation. Music starts off and can be enabled with the music icon.
17. In editor mode, right-click a tile and verify Clear tile plus the alternating Hide tile/Show tile action. Hide must make the complete tile black; Show must restore its authored content.
